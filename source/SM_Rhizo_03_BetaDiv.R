...
Sample_type <-ggplot(Ordination,aes(x=PC1,y=PC2,colour =Plant.type,shape=Plant.type))+
  ...
  geom_label_npc(data= df.annotations , 
                 aes(npcx = "right", npcy = "bottom", label = label),
                 parse=T,size=4)+
  labs(title="Rhizosphere - Sample type effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
  theme(legend.position = "bottom",
...
Time <- ggplot(Ordination,aes(x=PC1,y=PC2,fill =Time))+
  ...
  geom_label_npc(data= df.annotations , 
                 aes(npcx = "right", npcy = "bottom", label = label),
                 parse=T,size=4)+
  labs(title="Rhizosphere - Time effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
...
Temperature <- ggplot(Ordination,aes(x=PC1,y=PC2,fill =Temperature))+
  ...
  geom_label_npc(data= df.annotations , 
                 aes(npcx = "right", npcy = "bottom", label = label),
                 parse=T,size=4)+
  labs(title="Rhizosphere - Temperature effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
...
### Save figure 
Rhizosphere <- ggarrange(Sample_type,Temperature,Time,
                   labels="AUTO",
                   ncol=1,
                   legend = "bottom")
...
write.table(Pairwise.time,row.names=T,sep=";",here("Results","Tables","Rhizo_RCLR_PWP_Time.csv"))
...
Pairwise.plant_temp
write.table(Pairwise.plant_temp,row.names=T,sep=";",here("Results/Tables/Interactions","Rhizo16S_RCLR_PWP_Plant_Temp.csv"))
...
Pairwise.time_temp
write.table(Pairwise.time_temp,row.names=T,sep=";",here("Results/Tables/Interactions","Rhizo16S_RCLR_PWP_Time_Temp.csv"))
...
Pairwise.plant_time
write.table(Pairwise.plant_time,row.names=T,sep=";",here("Results/Tables/Interactions","Rhizo16S_RCLR_PWP_Time_Plant.csv"))
