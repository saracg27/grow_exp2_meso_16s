...
write.table(Permanova.Warm.rclr,row.names=T,sep=";",
            here("Results_W&C","Tables","Sed_Warm_RCLR_Permanova.csv"))
...
# ggsave(here("Results_W&C","Figures","Sed_Warm_RCLR_Ordination.pdf"),device='pdf',height = 7.5, width = 10.5)
# ggsave(here("Results_W&C","Figures","Sed_Warm_RCLR_Ordination.png"),device='png',height = 7.5, width = 10.5)
...
Pairwise.PT <- pairwise.adonis(rclr_dist_matrix, 
                               ...
                               p.adjust.m ="fdr")
Pairwise.PT
# All plant types significantly different from one another

Pairwise.Time<- pairwise.adonis(rclr_dist_matrix, 
                                metadata$Time,
                                p.adjust.m ="fdr")
Pairwise.Time
# D62 significantly different from all other sampling times 
# D35 different from D16 and D0
...
write.table(Permanova.Cold.rclr,row.names=T,sep=";",
            here("Results_W&C","Tables","Sed_Cold_RCLR_Permanova.csv"))
...
# ggsave(here("Results_W&C","Figures","Sed_Cold_RCLR_Ordination.pdf"),device='pdf',height = 7.5, width = 10.5)
# ggsave(here("Results_W&C","Figures","Sed_Cold_RCLR_Ordination.png"),device='png',height = 7.5, width = 10.5)
...
RCLR_WC <- ggarrange(RCLR_Warm,RCLR_Cold,
                     nrow=2)
ggsave(here("Results_W&C","Figures","Sed_W&C_RCLR_Ordination.pdf"),
       plot=RCLR_WC, device='pdf',height = 7.5, width = 10.5)

ggsave(here("Results_W&C","Figures","Sed_W&C_RCLR_Ordination.png"),
...
Sample_type <-ggplot(Ordination,aes(x=PC1,y=PC2,colour =Plant.type,shape=Plant.type))+
  ...
  geom_label_npc(data= df.annotations , 
                 ...
                 parse=T,size=4)+
  labs(title="CST - Sample type effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
...
Temperature <- ggplot(Ordination,aes(x=PC1,y=PC2,fill =Temperature))+
  ...
  geom_label_npc(data= df.annotations , 
                 aes(npcx = "right", npcy = "bottom", label = label),
                 parse=T,size=4)+
  labs(title="CST - Temperature effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
...
### Save figure 
CST <- ggarrange(Sample_type,Temperature,Time,
                       labels="AUTO",
                       nrow=1,
                       legend = "bottom")

ggsave(here("Results","Figures","Figure_B_CST_beta_div.svg"),device='svg',height = 7, width = 18)
...
write.table(Pairwise.time,row.names=T,sep=";",here("Results/Tables/","Sed_RCLR_PWP_Time.csv"))
