...
pdf(file = here("output", "figures", "heteroatoms-selection_heatmap_totalabund.pdf"))
...
pdf(file = here("output", "figures", "heteroatoms-selection_heatmap_totalabund2.pdf"))
...
```{r}
set.seed(123)
rda2 <- dbrda(otu_hell_dist ~ ., 
                 data = atoms_reduced2)
anova(rda2, permutations = how(nperm = 999))
summary(rda2)
print(rda2)
adjR2.tbrda <- RsquareAdj(rda2)$adj.r.squared #0.0262 adjusted R2 explained
```
...
rda0_1 <- dbrda(otu_hell_dist ~ 1, data = meta_g[,1:4])
rdaall_1 <- dbrda(otu_hell_dist ~ Plant.type + `NAFCs (mg/L)` + Temperature, data = meta_g[,1:4]) # exact same as rda2

anova(rdaall_1, permutations = how(nperm = 999))
summary(rdaall_1)
print(rdaall_1)
adjR2.tbrda1 <- RsquareAdj(rdaall_1)$adj.r.squared #0.1099 adjusted R2 explained

rdaall_na <- dbrda(otu_hell_dist ~ `NAFCs (mg/L)`, data = meta_g[,1:4])
anova(rdaall_na, permutations = how(nperm = 999)) # Not significant on its own
...
# Create simplest and most complex models
rda0_full <- dbrda(otu_hell_dist ~ 1, data = meta_select)
rdaall_full <- dbrda(otu_hell_dist ~ ., data = meta_select) # exact same as rda2
...
sel_full$anova
sel_full$call
adjR2.tbrdaselected <- RsquareAdj(sel_full)$adj.r.squared #0.2723 adjusted R2 explained


# Perform RDA with selected variables
rda_select <- dbrda(formula = otu_hell_dist ~ Day + Plant.type, data = meta_select)
anova(rda_select, permutations = how(nperm = 999),by = "term")
print(rda_select)
adjR2.tbrda_select <- RsquareAdj(rda_select)$adj.r.squared #0.2723 adjusted R2 explained
