...
## DESeq2 genus
```{r}
# Subset the original phyloseq object 'ps_taxa' to only include samples from time points "D40" and "D66"
ps_taxa_sub <- subset_samples(ps_taxa, Time %in% c("D40", "D66")) # 48 samples
...
ps_taxa_pse_sub <- prune_taxa(rowSums(otu_table(ps_taxa_sub) == 0) < ncol(otu_table(ps_taxa_sub)) * 0.9, ps_taxa_sub) # 632
...
# Further subset the relative abundance phyloseq object to only include samples from time points "D40" and "D66"
ps_taxa_rel_sig <- prune_samples(colnames(otu_table(ps_taxa_pse_sub)), ps_taxa_rel_sig)
colSums(ps_taxa_rel_sig@otu_table)
```
...
ann_colors = list(
    ...
    `Time` = c(D40 = "purple", D66 = "yellow"),
    Phylum = phylum_col
)
png(here("output", "figures","heatmap_D40-D66-on-sediments_1.png"),width=8,height=4,units="in",res=600)
...
volcano_sed_40.66_genus <- EnhancedVolcano(res_volcano, 
                          lab = res_volcano$Genus, 
                          x = 'log2FoldChange', 
                          y = "padj",
                          title = "",
                          subtitle = "D40 vs D66 in Sediments - Genus",
                          pointSize = 4.0,
                          labSize = 3.0,
                          colAlpha = 0.5,
                          legendPosition = 'top',
...
volcano_sed_40.66_genus

ggsave(filename = here("output", "figures", "deseq_volcano_sed_40.66_genus.png"), plot = volcano_sed_40.66_genus, width = 6, height = 6, dpi = 300)
ggsave(filename = here("output", "figures", "deseq_volcano_sed_40.66_genus.pdf"), plot = volcano_sed_40.66_genus, width = 6, height = 6, dpi = 300)
...
```{r}
# Subset the original phyloseq object 'ps_taxa' to only include samples from time points "D40" and "D66"
ps_taxa_sub <- subset_samples(ps_taxa, Time %in% c("D40", "D66"))

# Filter out features (OTUs or taxa) that have more than 90% zeros across all samples
# This is done by checking the number of zeros in each row of the OTU table and comparing it to 90% of the total number of columns (samples)
ps_taxa_pse_sub <- prune_taxa(rowSums(otu_table(ps_taxa_sub) == 0) < ncol(otu_table(ps_taxa_sub)) * 0.9, ps_taxa_sub) # 225
...
volcano_sed_40.66_order <- EnhancedVolcano(res_volcano, 
                          lab = res_volcano$Order, 
                          x = 'log2FoldChange', 
                          y = "padj",
                          title = "",
                          subtitle = "D40 vs D66 in Sediments - Order",
                          pointSize = 4.0,
                          labSize = 3.0,
                          colAlpha = 0.5,
                          legendPosition = 'top',
...
volcano_sed_40.66_order
ggsave(filename = here("output", "figures", "deseq_volcano_sed_40.66_order.png"), plot = volcano_sed_40.66_order, width = 6, height = 6, dpi = 300)
ggsave(filename = here("output", "figures", "deseq_volcano_sed_40.66_order.pdf"), plot = volcano_sed_40.66_order, width = 6, height = 6, dpi = 300)
