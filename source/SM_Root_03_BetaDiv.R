...
Sample_type <-ggplot(Ordination,aes(x=PC1,y=PC2,colour =Plant.type,shape=Plant.type))+
  theme_bw()+
  geom_point(size=4)+
  
  scale_shape_manual(values=c(0,5),
                     name="Sample type",
                     labels=c(expression(italic("S. microcarpus")),expression(italic("T. maritima"))))+
  
  scale_colour_manual(values = c("slateblue4","chartreuse4"),
                      name="Sample type",
                      labels=c(expression(italic("S. microcarpus")),expression(italic("T. maritima"))))+
  ...
  geom_label_npc(data= df.annotations , 
                 ...
                 parse=T,size=4)+
  labs(title="Roots - Sample type effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
...
Temperature <- ggplot(Ordination,aes(x=PC1,y=PC2,fill =Temperature))+
  ...
  geom_label_npc(data= df.annotations , 
                 aes(npcx = "right", npcy = "bottom", label = label),
                 parse=T,size=4)+
  labs(title="Roots - Temperature effect",
       x=paste0("PC1 (",PC1,"%)"),
       y=paste0("PC2 (",PC2,"%)"))+
...
### Save figure 
Roots <- ggarrange(Sample_type,Temperature,Time,
                       labels=c("D","E","F"),
                       ncol=1,
                       legend = "bottom")


Figure_C_RootRhizo_beta_div <- ggarrange(Rhizosphere,Roots,
                                     labels=NULL,
                                     ncol=2)

ggsave(here("Results","Figures","Figure_C_RootRhizo_beta_div.svg"),device='svg',height = 18, width = 12)
...
write.table(Pairwise.time,row.names=T,sep=";",here("Results","Tables","Root_RCLR_PWP_Time.csv"))
...
Pairwise.plant_temp <- pairwise.adonis(rclr_dist_matrix, metadata$plant_temp,p.adjust.m ="fdr")
Pairwise.plant_temp
write.table(Pairwise.plant_temp,row.names=T,sep=";",here("Results/Tables/Interactions","Root16S_RCLR_PWP_Plant_Temp.csv"))
...
Pairwise.time_temp <- pairwise.adonis(rclr_dist_matrix, metadata$time_temp,p.adjust.m ="fdr")
Pairwise.time_temp
write.table(Pairwise.time_temp,row.names=T,sep=";",here("Results/Tables/Interactions","Root16S_RCLR_PWP_Time_Temp.csv"))
...
Pairwise.plant_time <- pairwise.adonis(rclr_dist_matrix, metadata$plant_time,p.adjust.m ="fdr")
Pairwise.plant_time
write.table(Pairwise.plant_time,row.names=T,sep=";",here("Results/Tables/Interactions","Root16S_RCLR_PWP_Time_Plant.csv"))
