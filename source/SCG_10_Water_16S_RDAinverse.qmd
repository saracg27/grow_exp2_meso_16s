...
Order rank in water samples. The time levels tested are d35 vs d62
...
```{r}
otu <- otu_table(as.matrix(counts_f), taxa_are_rows = TRUE)
tax <- tax_table(as.matrix(tax_clean))
sample <- sample_data(meta_f)
...
#otu_hell <- decostand(otu_g, method = "clr", pseudocount = 1)
#rowSums(otu_rclr)
...
pdf(file = here("output", "figures",
                "correlation-order-level-hell_16S-water.pdf"),
    height = 8)
...
```{r}
set.seed(123)
rda_select_hell <- rda(atoms ~ ` Frankiales` + ` Pirellulales`,
                      data = otu_hell) # 
...
differentiation of the two time points. This operation yielded 7
explanatory variables (orders normalized at relative abundance and then
square rooted --\> (hellinger transformation). We then proceeded to
...
RDA: *Frankiales* and *Pirellulales*. Members of the order
*Frankiales* (F = 11.4790, p \< 0.001) had the strongest effect,
whereas *Pirellulales* (F = 3.6026, p = 0.032) had a minor
contribution on NAFC heteroatom structure on water. The adjusted proportion
explained by the relative abundance of these two orders was 10.98% of
the total variance in heteroatom composition. The model was: formula =
atoms \~ *Frankiales* + *Pirellulales*. Although a small percentage, the variance explained by these two orders was significant (F = 7.5408, p \< 0.001, df = 2).
...
dim(biplot) # 2 2
rownames(biplot) <- c("italic(Frankiales)", "italic(Pirellulales)")
