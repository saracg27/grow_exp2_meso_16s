...
melt_corr <- reshape2::melt(corr_values) #1089

# Identify correlation pairs with high correlation values
correlation_pairs <- melt_corr[which(melt_corr$value > high_correlation_threshold & melt_corr$value < 1),] # 138
...
unique_data <- correlation_pairs_filtered %>%
  distinct(pair_id, .keep_all = TRUE) #69
...
capture.output(print(rda_select_min), file = here("output", "tables", "RDA_atoms-explained-by-actinomarinales.txt"))
capture.output(anova.cca(rda_select_min, permutations = how(nperm = 999)), file = here("output", "tables", "RDA_atoms-explained-by-actinomarinales_ANOVA.txt"))
capture.output(anova.cca(rda_select_min, permutations = how(nperm = 999), by = "term"), file = here("output", "tables", "RDA_atoms-explained-by-actinomarinales_ANOVAbyTerm.txt"))
capture.output(RsquareAdj(rda_select_min)$adj.r.squared, file = here("output", "tables", "RDA_atoms-explained-by-actinomarinales_adj_Rsq.txt"))
```
...
dim(biplot) # 2 2
rownames(biplot) <- c("italic(Actinomarinales)", "italic(OM190)")
...
rda.biplot
ggsave(filename = here("output", "figures", "rda_order_vars_.eps"), plot = rda.biplot, height = 5, width = 7)
```
...
rda.biplot.warm <- ggplot(data = warm, aes(x = RDA1, y = RDA2)) + 
  ...
  labs(title = "B) Warm")+
    ...
    guides(fill = guide_legend(override.aes=list(shape = 21), title = "Day"),
           shape = guide_legend(override.aes=list(fill = "grey40"))) 
rda.biplot.warm
ggsave(filename = here("output", "figures", "rda_order_vars_warm.eps"), plot = rda.biplot.warm, height = 5, width = 7)
```
...
rda.biplot.cold <- ggplot(data = cold, aes(x = RDA1, y = RDA2)) + 
  ...
  labs(title = "A) Cold")+
    ...
    guides(fill = guide_legend(override.aes=list(shape = 21), title = "Day"),
           shape = guide_legend(override.aes=list(fill = "grey40"))) 
rda.biplot.cold
ggsave(filename = here("output", "figures", "rda_order_vars_cold.eps"), plot = rda.biplot, height = 5, width = 7)
```

```{r}
library(ggpubr)
p <- ggarrange(rda.biplot.cold, rda.biplot.warm, common.legend = TRUE, legend = "right", nrow = 2)
ggsave(filename = here("output", "figures", "rda_order_vars_divided.eps"), plot = p, height = 6, width = 6.5)
```
