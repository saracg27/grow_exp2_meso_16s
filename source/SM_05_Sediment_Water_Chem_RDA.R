...
# 24 samples 13322 taxa



#### CLR transformation ####
## Apply a CLR transformation on un-agglomerated or agglomerated phyloseq objects

#ps.clr <- microbiome::transform(ps_sed_62, "clr") 
#ASV_clr <- as.data.frame(otu_table(ps.clr))
# 13322 ASVs

# NArm=F keeps the ASVs with a taxonomy of NA instead of discarding them

#ps_sed_62genus <- tax_glom(ps_sed_62,taxrank="Genus",NArm=F)
#ps.genus.clr <- microbiome::transform(ps_sed_62genus, "clr") 
#ASV_genus_clr <- as.data.frame(otu_table(ps.genus.clr))
# 956 genus (including NAs)

ps_sed_62order <- tax_glom(ps_sed_62,taxrank="Order",NArm=F)
ps.order.clr <- microbiome::transform(ps_sed_62order, "clr") 
ASV_order_clr <- as.data.frame(otu_table(ps.order.clr))
# 304 orders (including NAs)
...
sed_data <- read.table(file = "Data/Env_measures/env_final_sediment_exp2_20220225.csv", 
                         dec = ".", sep = ",")
colnames(sed_data) <- sed_data[1,] # Rename colnames (similar column name in original table)
sed_data <- sed_data[-1,-2] # removing first row (colnames)  and Temp column
colnames(sed_data)

####  Merge datasets ####

sdata <- data.frame(sample_data(ps_sed_62))
colnames(sdata)
sdata <- select(sdata,c("CFL_ID_Sediments","Mesocosm..","Plant.type","Temperature","Greenhouse"))
...
RsquareAdj(RDA) # adj.r.squared = xx for un-agglomerated ASVs
# adj.r.squared = 0.1266618 for genus agglomerated ASVs
# adj.r.squared = 0.1915653 for order agglomerated ASVs

#adj.r.squared = 0.1613894 for order agglomerated ASVs - Robust Ait

fwd.sel <- ordiR2step(rda(t(ASV_object) ~ 1, data = sed_rda_data[,4:21]), # modèle le plus simple
                      scope = formula(RDA), # modèle "complet"
                      direction = "forward",
                      R2scope = TRUE, # limité par le R2 du modèle "complet"
                      #pstep = 1000,
...
fwd.sel$call
# Ait
# xx for un-aglomerated ASVs
# pH for genus agglomerated ASVs
# pH  for order agglomerated ASVs

# robust Ait
# pH  for order agglomerated ASVs

anova.cca(RDA, permutations = 1000) # Not significant for genus, 0.02897 for order
anova.cca(RDA, permutations = 1000,by="term")
# xx for un-agglomerated
# pH and `Sulfate-S_mg/kg` for genus agglomerated
# pH , `Sulfate-S_mg/kg` and Chloride_mg/kg` for order agglomerated

RDA_signif <- vegan::rda(t(ASV_object) ~ pH , data = sed_rda_data[,4:21])
RsquareAdj(RDA_signif) #0.02221067 for genus / 0.03120386 for order
#0.03302412 for Rclr orde glom
anova.cca(RDA_signif, permutations = 999) #0.035 for genus / 0.024 for order 
...
Rda.plot.clr.S2<-ggplot(data=coord.sites.rda, aes(x=RDA1, y=PC1)) +
  theme_bw()+
  geom_point()+
  geom_text_repel(label=rownames(coord.sites.rda))+
